...
import { PALETTES, TYPE_PAIRINGS, CIRCLES } from "../data/mockData";
import {
  useTweaks,
  TweaksPanel,
  TweakSection,
...
function ChatLobby({ userSession }) {
  const [t, setTweak] = useTweaks(TWEAK_DEFAULTS);
  const palette = PALETTES[t.palette] || PALETTES.garden;
  const type = TYPE_PAIRINGS[t.typeface] || TYPE_PAIRINGS.literary;
...
function PaletteTweak({ value, onChange }) {
  ...
  const opts = keys.map((k) => PALETTES[k].map((h) => h.base));
  const cur = PALETTES[value].map((h) => h.base);
  const onPick = (arr) => {
    const k = keys.find(
      (key) => PALETTES[key][0].base.toLowerCase() === arr[0].toLowerCase(),
    );
    if (k) onChange(k);
  };
  return (
